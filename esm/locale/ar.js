...
const locale = {
  ...
  preparse(string) {
    return string.replace(fromArabNumeralsRegex, match => numberMap[match]).replace(fromArabComaRegex, ',');
  },

  postformat(string) {
    return string.replace(toArabNumeralsRegex, match => symbolMap[match]).replace(toArabComaRegex, '،');
  },
